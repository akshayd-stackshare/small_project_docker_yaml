...
def download_url(url: str, correlation_id: UUID) -> URLMetadata:
    with sync_playwright() as p:
        ...
        har_upload_to_minio(file_path=har_path, object_name=har_object_name)
        ...
        screenshot_upload_to_minio(screenshot_bytes, object_name=screenshot_object_name)
        ...
        html_upload_to_minio(io.StringIO(html_src), object_name=html_object_name)
...
if __name__ == '__main__':
    ...
    for key, urls in news_urls.items():
        ...
        for _url in urls:
            count += 1
            _correlation_id: UUID = uuid.uuid4()
            try:
